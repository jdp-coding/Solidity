...
contract TEST_20250103 {

    mapping(string => bytes32) private idPass;


    // * 로그인 기능 - ID, PW를 넣으면 로그인 여부를 알려주는 기능
    function register(string memory _id, string memory _password) public returns(string memory) {

        bytes32 hashedIDPassword = keccak256(abi.encodePacked(_id, _password));

        idPass[_id] = hashedIDPassword;
