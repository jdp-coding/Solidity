...
contract TEST_20250113 {
    ...
    uint public taxRate = 1; // 1% tax
    mapping(string => Bank) public banks;
    mapping(address => uint) public pendingTaxes;


    function addBank(string memory bankName) public {
        require(!banks[bankName].exists, "Bank already exists");
        banks[bankName].exists = true;
    }

    function registerUser(string memory bankName) public {
        require(!banks[bankName].exists, "Bank already exists");
        require(!banks[bankName].users[msg.sender].exists, "User already registered in this bank");
        banks[bankName].users[msg.sender] = User(0, true);
    }
